...
function generateRandomIntWithRange(min, max) {
...
for (let i = 0; i < 20; i++) {
    arr4[i] = generateRandomIntWithRange(8, 732);
}
...
//     if ((i + 1) % 3 === 0 && i !== 0)
//         console.log(arr4[i] + ' with index => ' + i);
// }
...
for (let i = 0; i < arr4.length; i++) {
    if ((arr4[i + 1]) % 2 === 0) {
        console.log(arr4[i] + ' and its next => ' + (arr4[i + 1]));
    }
}
...
const averageCheckCalc = (sum, quantity) => {
...
console.log(averageCheckCalc(averageCheck, arr6.length));
...
const arr9 = [1, 2, '3', '4', true, false, undefined, {a: '1'}, 5];
...
for (const user of usersList) {
    ...
    let userBlock_h2 = document.createElement('h2');
    userBlock_h2.innerText = user.id + ' - ' + user.name + ' - ' + user.username;
    let userBlock_h3 = document.createElement('h3');
    userBlock_h3.innerText = user.email + ' - ' + user.phone;
    let addressBlock = document.createElement('div');
    addressBlock.setAttribute('class', 'address-block')
    userBlock.append(userBlock_h2, userBlock_h3, addressBlock);
